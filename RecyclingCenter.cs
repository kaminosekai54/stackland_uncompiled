...
public class RecyclingCenter : Factory
{
	...
	public override void UpdateCardText()
	{
		if (!this.IsOverflowing)
		{
			...
			{
				base.descriptionOverride = SokLoc.Translate("card_recycling_center_description", LocParam.Create("amount", this.PollutionOverflowMin.ToString()));
			}
			else
			{
				base.descriptionOverride = SokLoc.Translate("card_recycling_center_description_long", LocParam.Create("amount", this.PollutionOverflowMin.ToString()), LocParam.Create("current", this.StoredPollution.ToString()));
			}
		}
		else
		{
			base.nameOverride = SokLoc.Translate("card_overflowing_recycling_center_name");
			base.descriptionOverride = SokLoc.Translate("card_overflowing_recycling_center_description");
		}
	}
	...
	public override void UpdateCard()
	{
		if (!this.IsOverflowing)
		{
			...
			{
				base.MyGameCard.StartTimer(30f, DumpPollution, SokLoc.Translate("card_landfill_status_1", LocParam.Create("amount", this.PollutionRemovalRate.ToString())), base.GetActionId("DumpPollution"));
			}
